...
export const getUserInput = async () => {
  ...
  const username = await _getAnswer(
    'Enter your Spotify username: ',
    'username'
  );
  const playlistName = await _getAnswer(
    'Enter the name for the new playlist: ',
    'playlist name'
  );
  const playlistDescription = await _getAnswer(
    'Enter the description for the new playlist: ',
    'playlist description'
  );
  const isPublic = await _getAnswer(
    'Should the playlist be public? (yes/no): ',
    'public status'
  );

  let splitTracks: string | number = await _getAnswer(
  ...
  if (splitTracks < 1 || splitTracks > 11000) {
    ...
    console.log(
      '\n\x1b[33m%s\x1b[0m',
      'Number of tracks out of range. Defaulting to 11000.'
    );
    splitTracks = 11000;
  }
