...
const start = async () => {
  ...
  for (const album of albums) {
    ...
    let trackData = await getAlbumTracks(
      accessToken.access_token,
      album.id,
      offset,
      limit
    ...
    while (totalTracks > 0) {
      ...
      trackData = await getAlbumTracks(
        accessToken.access_token,
        album.id,
        offset,
        limit
  ...
  while (totalPlaylists > 0) {
    ...
    const currentPlaylistName =
      playlistName + (totalPlaylists > 1 ? ` ${currentPlaylist}` : '');
    ...
    if (playlist?.id) {
      ...
      let tracksToAdd = tracks
        .map(track => track.uri)
        .slice(tracksOffset, tracksOffset + requestLimit);
      let playlistOffset = 0;

      while (totalTracks > 0) {
        console.log(
          `Adding ${tracksToAdd.length} tracks to playlist '${playlist.name}': Offset: ${playlistOffset}, Tracks Left: ${totalTracks}`
        );
    ...
    if (totalPlaylists > 0) {
      totalTracks =
        splitTracks > tracks.length - tracksOffset
          ? tracks.length - tracksOffset
          : splitTracks;
    }
  }
